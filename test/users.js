...
describe('users API', function () {
  ...
  it('DELETE /users/:id', function () {
    const user_id = 1;
    return request.delete(`/users/${user_id}`).expect(200);
  });

  it('using the response from one call in the request for another', function () {
    return request
      .get('/users') //make first request to get all the users
      .expect(200)
      .then(function (res) {
        // get the user id of first user and make second call to update his email
        user_id = res.body[0].id;
        return request
          .put(`/users/${user_id}`)
          .send({ email: 'sampleEmail@jsonplaceholder.com' })
          .expect(200);
      });
  });
  ...
  it('POST-create new user verify new user in GET all users list', function () {
    ...
    return request
      ...
      .then(function (res) {
        ...
        return request.get('/users').then(function (res) {
          //make GET all users API CALL
          ...
          assert.isNotOk(
            all_users_id.includes(new_id),
            'given user id does exist'
          ); // asserts that result should be false (i.e. id should not be in the list as this is fake user creation)
        });
      });
  });
});
